...
def year_pass():
    ...
    destroy = []
    prey_list = entities_shuffled.copy()                                                 #All living creatures are potential preys
...
def create_entity(name, entities, amount, saturation = 100):
    try:
        parameters[0] = str(name)
        parameters[1] = 0
...
def main():
    ### Creating entities ###
    name = input('\nDo you want to \033[31mCreate\033[0m entities? (Enter to pass, kind name (0 to 11) or "RND" to go): ')
    try:
        if len(name) == 0:
            pass
        elif name == "RND":
            for i in range(0,12):
                ...
                create_entity(i, entities, amount)
    ...
    if len(name) >= 1:
        if name == 'ALL':
            ...
            for i in range(0,12):
                if len(recall_entity(i, entities)) != 0:
                    print(*(recall_entity(i, entities)), sep='\n')
    ...
    name = input('\nDo you want to \033[33mInitiate reproduction\033[0m? (Enter to pass, kind name (0 to 11) or "RND" to go): ')
    if len(name) >= 1:
        if name == 'RND':
            name = random.choice(range(0,12))
